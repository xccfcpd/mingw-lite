...
namespace mingw_thunk
{
                 ...
                 _startupinfo *start_info)
  {
    ...
    char **argv = (char **)internal::early_malloc(sizeof(char *) * (argc + 1));
    for (int i = 0; i < argc; i++) {
      int len = WideCharToMultiByte(
          CP_UTF8, 0, wargv[i], -1, nullptr, 0, nullptr, nullptr);
      argv[i] = (char *)internal::early_malloc(len);
      WideCharToMultiByte(
  ...
  namespace internal
  {
    u8_envp_result u8_envp_from_wenvp(wchar_t **wenvp)
    {
      ...
      char **envp =
          (char **)internal::early_malloc(sizeof(char *) * (envc + 1));
      for (int i = 0; i < envc; i++) {
        int len = WideCharToMultiByte(
            CP_UTF8, 0, wenvp[i], -1, nullptr, 0, nullptr, nullptr);
        envp[i] = (char *)internal::early_malloc(len);
        WideCharToMultiByte(
    ...
    u8_envp_result u8_envp_from_win32_env_strings()
    {
      ...
      for (int i = 0; i < envc; i++) {
        ...
        envp[i] = (char *)internal::early_malloc(u_len + 1);
        WideCharToMultiByte(
