...
function msvcrt_thunk_toolchain_w()
  return {
    {'_wchdir'},
    {'_wchmod'},
    {'_wfindfirst'},
...
target('overlay-msvcrt-os')
  ...
  if profile_core() then
    if ntddi_version() < ntddi_win4() then
      ...
      add_msvcrt_sources(msvcrt_thunk_core_w(), 'msvcrt/w')
    end
  ...
  if profile_toolchain_or_utf8() then
    if ntddi_version() < ntddi_win4() then
      ...
      add_msvcrt_sources(msvcrt_thunk_toolchain_w(), 'msvcrt/w')
    end
    if ntddi_version() < ntddi_win2k() then
  ...
  if profile_toolchain_utf8() then
    add_files(
      ...
      'msvcrt/u/runtime/@early_alloc.cc',
