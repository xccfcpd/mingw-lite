...
from .util import OPT_LV_2_XMAKE_MODE_MAP, XMAKE_ARCH_MAP, add_objects_to_static_lib, common_cross_layers, ensure, extract_shared_libs, overlayfs_ro, remove_info_main_menu, strip_pe_tls_directory
...
def _xmake(ver: BranchProfile, paths: ProjectPaths, config: argparse.Namespace):
  # TODO: xmake 3.0.9 failed to read lua file >64 KiB in debug mode
  opt_lv = OptLv.O1 if ver.opt_lv in (OptLv.O0, OptLv.Og) else ver.opt_lv
  ...
  with overlayfs_ro('/usr/local', [
    ...
    *common_cross_layers(paths),
    ...
    xmake_config(build_dir, [
      '--plat=mingw',
      f'--arch={XMAKE_ARCH_MAP[ver.arch]}',
      f'--mode={OPT_LV_2_XMAKE_MODE_MAP[opt_lv]}',
      '--mingw=/usr/local',
    ])
